...
pipeline {
    ...
    stages {
        ...
        // Always run: gives a host-by-host drift report even when no changes detected.
        stage('Fleet Preflight (Drift Report)') {
            steps {
                script {
                    def hosts = [
                        [ssh: 'genie@10.114.1.111', label: 'genie-os'],
                        [ssh: 'genie@10.114.1.121', label: 'cegonha'],
                        [ssh: 'genie@10.114.1.124', label: 'stefani'],
                        [ssh: 'genie@10.114.1.126', label: 'gus'],
                        [ssh: 'genie@10.114.1.131', label: 'luis'],
                        [ssh: 'genie@10.114.1.154', label: 'sampaio'],
                        [ssh: 'openclaw@10.114.1.119', label: 'juice'],
                        [ssh: 'genie@10.114.1.140', label: 'omni-prod'],
                    ]

                    def failures = []
                    for (h in hosts) {
                        echo "Preflight: ${h.label} (${h.ssh})"
                        def rc = sh(
                            script: """
                                ssh -o BatchMode=yes -o ConnectTimeout=10 ${h.ssh} 'bash -s -- ${h.label}' < ${PREFLIGHT_SCRIPT}
                            """,
                            returnStatus: true
                        )
                        if (rc != 0) {
                            echo "PREFLIGHT_FAIL: ${h.label} (rc=${rc})"
                            failures.add("${h.label}(rc=${rc})")
                        }
                    }

                    if (failures.size() > 0) {
                        currentBuild.description = "Preflight drift/health issues: ${failures.join(', ')}"
                        error("Fleet preflight failed on: ${failures.join(', ')}")
                    } else {
                        echo 'Fleet preflight OK on all hosts'
                    }
                }
            }
        }
